...
from tasks.persona_inference.agents import PersonaInferenceTeacher
from tasks.emotion_inference.agents import EmotionInferenceTeacher
from tasks.topic_inference.agents import TopicInferenceTeacher
...
def _eval_single_world(opt, agent, task):
    ...
    if 'persona_inference' in task_opt['task']:
        task_agent = PersonaInferenceTeacher(opt=task_opt)
    elif 'topic_inference' in task_opt['task']:
        task_agent = TopicInferenceTeacher(opt=task_opt)
    elif 'emotion_inference' in task_opt['task']:
        task_agent = EmotionInferenceTeacher(opt=task_opt)
    tfidf_agent = TfidfRetrieverAgent(opt=task_opt)
    world = DialogPartnerWorld(opt=task_opt, agents=[task_agent, tfidf_agent])
    world = BatchWorld(task_opt, world)
